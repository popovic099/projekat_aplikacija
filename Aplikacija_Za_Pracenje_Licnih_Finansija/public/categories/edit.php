...
// Proveri da li kategorija postoji i pripada korisniku
if (!$category || !$categoryRepo->belongsToUser($id, $user['id'])) {
    Session::flash('danger', 'Kategorija nije pronađena.');
    redirect('/categories/');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            Session::flash('success', 'Kategorija je ažurirana.');
            redirect('/categories/');
        } catch (Exception $e) {
...
<h1 class="mb-4">Izmena kategorije</h1>
                ...
                <form method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="type" name="type" required>
                            <option value="prihod" <?= $category['type'] === 'prihod' ? 'selected' : '' ?>>
                                Prihod
                            </option>
                            <option value="rashod" <?= $category['type'] === 'rashod' ? 'selected' : '' ?>>
                                Rashod
                            </option>
                        </select>
                    </div>
